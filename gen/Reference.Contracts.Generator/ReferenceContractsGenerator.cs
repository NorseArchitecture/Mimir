...
namespace Norse.Reference.Contracts.Generator;

/// <summary>
/// Parses the UNSD M49 raw CSV, carried into the compilation as an <see cref="AdditionalText"/>, and
/// emits the <c>Norse.Reference.IsoCountryCode</c> enum plus its tri-form span-based parser. Reports
/// <c>MIMIR001</c> when the header is missing an expected column and <c>MIMIR002</c> when two rows
/// sanitize to the same identifier — both fail loud rather than emit a partial or ambiguous surface.
/// </summary>
[Generator]
public sealed class ReferenceContractsGenerator : IIncrementalGenerator
{
	...
	static readonly DiagnosticDescriptor _identifierCollision = new(
		"MIMIR002", "Sanitized country identifier collides with another row",
	...
	public void Initialize(IncrementalGeneratorInitializationContext context)
	{
		...
		context.RegisterSourceOutput(csvTexts, static (ctx, csvText) => Emit(ctx, csvText));
	}
