...
namespace Norse.Reference.Contracts.Generator.Tests;
...
static class GeneratorTestHarness
{
	...
	internal static string Run(string csv) =>
		string.Join("\n", Execute(csv).Result.GeneratedTrees.Select(tree => tree.ToString()));

	/// <summary>Runs the generator over <paramref name="csv"/> and returns the resulting compilation.</summary>
	internal static Compilation RunAndCompile(string csv) =>
		Execute(csv).OutputCompilation;

	static (Compilation OutputCompilation, GeneratorDriverRunResult Result) Execute(string csv)
	{
		// The real Reference.Contracts.csproj carries ImplicitUsings=enable and LangVersion=preview
		// (Mimir's root Directory.Build.props) — both matter to the *emitted* source, which relies on
		...
		ReferenceContractsGenerator generator = new();
	...
	static Compilation CreateCompilation(CSharpParseOptions parseOptions)
	{
